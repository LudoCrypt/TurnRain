...
/*
Shamelessly stolen from POM
*/

internal static class Logfix
{
	private const int MAX_BUFFERED_LINES = 4096;
	private static System.Collections.Generic.Queue<BufferedLogMessage> __bufferedLogMessages = new(MAX_BUFFERED_LINES);
	...
	internal static void __SwitchToBepinexLogger(BepInEx.Logging.ManualLogSource logger)
	{
		...
		while (__bufferedLogMessages.Count > 0)
		{
			...
			(BepInEx.Logging.LogLevel level, string data) = result;
			Log(level, $"[BUFFERED : ] {data}");
		}
	...
	private static void __DefaultImpl_Log(BepInEx.Logging.LogLevel level, object data)
	{
		...
		__bufferedLogMessages.Enqueue(new(level, __GenerateLogString_Bepinex(level, data)));
	}
	internal static string __GenerateLogString_Unity(BepInEx.Logging.LogLevel level, object data) => $"[POM/{level}] {data}";
	...
	private record struct BufferedLogMessage(BepInEx.Logging.LogLevel level, string text);
}
